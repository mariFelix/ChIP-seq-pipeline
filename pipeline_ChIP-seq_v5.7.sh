...
# 2016-02-29
# 2016-03-02
# 2017-08-07 : remove continue from case in
# 2018-03-26 : Change samtools sort command (update)
# 2018-06-26 : Change conversion step bam -> fastq.gz from Bedtools to Picard
# 2019-10-31 : Change output (ease the transfer)
#
# pipeline_ChIP-seq_v5.7.sh
...
- The device must have enough space to stock two times the size of each file.

==============
 OUTPUT FILES
==============

There will be a folder named yearMonthDay_hourMinSec. This forlder will
contain a metadata file in which a log, a summary and the quality check
folders. It will also contain some k* files with the *.bam and *.bai
files in it.

Here is the type of files contained in the yearMonthDay_hourMinSec folder :

- metadata/
- k*/*.bam

Here is the type of files contained in the metadata folder :

- yearMonthDay_hourMinSec.log
- settingsSummary-yearMonthDay_hourMinSec.txt
- QualityCheck-yearMonthDay_hourMinSec/*_fastqc
...
Toolkit or Picard). Then, a FastQC quality analysis will be performed on these
...
*******************************************
*                                         *
*$normal   ChIP-seq pipeline version 5.6         $bold*
*   -----------------------------------   $bold*
...
echo "==============================" >> $summary
echo "" >> $summary
echo "ChIP-Seq pipeline version 5.7" >> $summary
...
QualityCheck=QualityCheck-$(date +%Y%m%d_%H%M%S)
...
do
    ...
    case "${file##*.}" in
        ...
        bam)
            pathPicard=`locate picard-tools-1.138/picard.jar`
            outputFastq=`basename "${file%.*}"`.fastq
            java -jar $pathPicard SamToFastq I=$file FASTQ=$outputFastq  &>> $log
            gzip $outputFastq
    ...
    do
        ...
        samtools sort -o FinalFiles/$sortedFile.bam -O bam -@ $threads  \
        -T $sortedFile $alnFile &>> $log
...
mkdir metadata
mv $QualityCheck $log $summary metadata
...
echo "Experiment $experiment used $space of additional disk space on your device" | tee -a $experiment/metadata/$log

echo "and was processed in $(($ElapsedTime / 3600 )) hours $((($ElapsedTime % 3600) / 60)) \
minutes $(($ElapsedTime % 60)) seconds !" | tee -a $experiment/metadata/$log

transition
echo "--------------------------------------------------------------------------------" >> $experiment/metadata/$log

echo "Final files are in folder $experiment/k* ! :)" | tee -a $experiment/metadata/$log
